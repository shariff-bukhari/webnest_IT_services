...
// Update remaining spots randomly (for demo)
function updateRemainingSpots() {
    const spotsLeft = Math.floor(Math.random() * 6) + 3; // Random between 3-8
    const spotElements = document.querySelectorAll('.offer-badge, .offer-mini-badge, .offer-sidebar div');
    spotElements.forEach(el => {
        if (el.textContent.includes('left') || el.textContent.includes('Left') || el.textContent.includes('Spots')) {
            el.innerHTML = `<i class="fas fa-bolt"></i> Only ${spotsLeft} Spots Left!`;
        }
    });
}
...
// Optional: Show popup after scrolling
window.addEventListener('scroll', function() {
    const offerPopup = document.getElementById('offerPopup');
    if (offerPopup) {
        const scrollPercent = (window.scrollY / (document.documentElement.scrollHeight - window.innerHeight)) * 100;
        if (scrollPercent > 30 && offerPopup.style.display === 'none') {
